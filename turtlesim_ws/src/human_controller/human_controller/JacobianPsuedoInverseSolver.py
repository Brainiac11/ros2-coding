...
class JacobianPsuedoInverseSolver:
    ...
    def __init__(
        ...
        self._end_effector_location_cartesian = self.find_end_effector_location_cartesian()
    
    def jacobian_gradient_descent(self) -> NDArray:
        """Jacobian Psuedoinverse (Dampended Least Squares)
        ...
        for i in range(100000):
            ...
            if(self.find_error_squared_magnitude() < self.MAX_ALLOWABLE_ERROR):
                ...
                break
            error_vector: NDArray = self.find_error_vector()[: 2] # only x and y are needed in the entire thing
        
            dq = self.EPSILON_CONSTANT *  np.dot(np.linalg.pinv(J), error_vector)
            self._link_local_angles += dq
            self._end_effector_location_cartesian = self.find_end_effector_location_cartesian()
            if i % 100 == 0:
                print(f"Iter {i}: |error|={self.find_error_squared_magnitude():.5f}, FK={self._end_effector_location_cartesian}")
    ...
    def compute_jacobian(self, delta = 1e-6) -> NDArray:
        ...
        for j in range(link_num):
            ...
            for i in range(j, link_num):
                angle_sum = np.sum(self._link_local_angles[: i+ 1]) # final angle after ading up all the global angels
                dx_dtheta -= self._link_lengths[i] *np.sin(angle_sum) # derivative of cos is -sin
                dy_dtheta += self._link_lengths[i] * np.cos(angle_sum) # derivative of sin is cos
            J[0, j] = dx_dtheta
            J[1, j] = dy_dtheta
        print("jacobian_gradient_descent finished")
        return J
