...
class KickController(Node):
    def __init__(self):
        ...
        self._knee_pitch_subscription = self.create_subscription(
            Float64, "/LKneePitch", self._knee_pitch_callback, 1
        )
        self._ankle_pitch_subscription = self.create_subscription(
            Float64, "/LAnklePitch", self._ankle_pitch_callback, 1
        )
        self._hip_pitch_subscription = self.create_subscription(
            Float64, "/LHipPitch", self._hip_pitch_callback, 1
        )

        self._knee_pitch_publisher = self.create_publisher(
            Float64, "/LKneePitch", qos_profile=1
        )
        self._ankle_pitch_publisher = self.create_publisher(
            Float64, "/LAnklePitch", qos_profile=1
        )
        self._hip_pitch_publisher = self.create_publisher(
            Float64, "/LHipPitch", qos_profile=1
        )

        self._should_kick_subscription = self.create_subscription(
            KickCommand, "/should_kick", self._should_kick_callback, 1
        )
