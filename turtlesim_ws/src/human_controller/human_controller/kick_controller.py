...
class KickController(Node):
    def __init__(self):
        ...
        self._knee_roll_subscription = self.create_subscription(Float64, '/LKneeRoll', self._hip_roll_callback, 1)
        self._ankle_pitch_subscription = self.create_subscription(Float64, '/LAnklePitch', self._hip_yaw_pitch_callback, 1)
        self._hip_pitch_subscription = self.create_subscription(Float64, '/LHipPitch', self._hip_pitch_callback, 1)

        self._knee_roll_publisher = self.create_publisher(Float64, '/LKneeRoll', qos_profile=1)
    ...
    def _hip_roll_controller(self, x: float) -> Float64:
        self._knee_roll_publisher.publish(Float64(data=x))
