...
class IKTester(Node):
    ...
    def send_ik_request(
        ...
        if fix_j1 or fix_j2 or fix_j3:
            self.get_logger().info(
                f"Fixed joints- J1={fix_j1}, J2={fix_j2}, J3={fix_j3}"
            )
        ...
        if response.success:
            ...
            self.get_logger().info(
                f"joint 1 {response.joint1_angle:.4f} degrees: ({math.degrees(response.joint1_angle):.2f})"
            )
            self.get_logger().info(
                f"joint 2 {response.joint2_angle:.4f} degrees: ({math.degrees(response.joint2_angle):.2f})"
            )
            self.get_logger().info(
                f"joint 3 {response.joint3_angle:.4f} degrees: ({math.degrees(response.joint3_angle):.2f})"
            )
        else:
...
def main(args=None):
    ...
    client.send_ik_request(
        target_x=1.5, target_y=1.0, target_z=0.0, l1=2.0, l2=1.0, l3=0.5
    )
    ...
    client.send_ik_request(
        target_x=5.0, target_y=5.0, target_z=5.0, l1=1.0, l2=1.0, l3=0.5
    )
